...
# This module implements the LogLog and HyperLogLog algorithms for estimating the number of distinct elements in a stream.
def logLog_estimate(stream:array, M:int)->int:
    '''Estimate the number of distinct elements in a stream using the LogLog algorithm.'''
    sketch = array('i', [0] * M)  # Initialize an array to keep track of the maximum rank for each bucket
    ...
    for k in range(M):
        total+=r(sketch[k])
    mean = 1.0 * total / M
    return int(M * math.pow(2, mean)/0.77351)  # Return the estimated count as an integer


def hyperlogLog_estimate(stream:array, M:int)->int:
    """
    Estimate the number of distinct elements in a stream using the HyperLogLog algorithm.
